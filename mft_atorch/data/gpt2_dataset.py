...
class GPT2PromptDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.loss_mask_indexed_dataset = loss_mask_indexed_dataset
        ...
        if train_mode == 'sft':
            ...
            self.task_name = data_prefix.split('/')[-2]
        self.task_id = TASK2ID[self.task_name]

        # Checks
        assert np.min(documents) >= 0
        assert np.max(documents) < self.input_ids_indexed_dataset.sizes.shape[0]
    ...
    def __getitem__(self, idx):
        try:
            ...
            if doc_index_f == doc_index_l:
                input_ids = self.input_ids_indexed_dataset.get(
                ...
                if self.loss_mask_indexed_dataset is not None:
                    loss_mask = self.loss_mask_indexed_dataset.get(
                        self.doc_idx[doc_index_f],
                        offset=offset_f,
                        length=offset_l - offset_f + 1,
                    )
                ...
                for i in range(doc_index_f + 1, doc_index_l):
                    input_ids_list.append(self.input_ids_indexed_dataset.get(self.doc_idx[i]))
                    if self.loss_mask_indexed_dataset is not None:
                        loss_mask_list.append(self.loss_mask_indexed_dataset.get(self.doc_idx[i]))
                ...
                if self.loss_mask_indexed_dataset is not None:  # sst do not need to read loss_mask.bin
            ...
            if self.weighted_loss_mode:
                ...
                #     "weight": np.array([self.ds_weight], dtype=np.float32),
                # }
                if self.loss_mask_indexed_dataset is not None:
                    res = {
                        "input_ids": np.array(input_ids, dtype=np.float32),
                        "loss_mask": np.array(loss_mask, dtype=np.float32),
                        "weight": np.array([self.ds_weight], dtype=np.float32),
                        "task_id": np.array([self.task_id], dtype=np.int),
                    }
                else:
                    res = {
                        "input_ids": np.array(input_ids, dtype=np.float32),
                        "loss_mask": np.ones_like(input_ids),
                        "weight": np.array([self.ds_weight], dtype=np.float32),
                        "task_id": np.array([self.task_id], dtype=np.int),
                    }
            else:
...
def _build_index_mappings(
    ...
    if should_process_dataset:
        if (
            ...
            # 2.0这里做了更新：num_samples = num_epochs * token_per_epoch
            # 而1.0版本是：train_batch_size * train_iter * weighted_rate
