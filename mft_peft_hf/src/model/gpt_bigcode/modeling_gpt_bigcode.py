...
class GPTBigCodeAttention(nn.Module):
    ...
    def forward(
        ...
        if self.multi_query:
            ...
            key = key.view(bsz, q_len, 1, self.head_dim).transpose(1, 2)
            value = value.view(bsz, q_len, 1, self.head_dim).transpose(1, 2)
            # repeat k/v heads if n_kv_heads < n_heads
            key = repeat_kv(key, self.num_heads).transpose(1, 2)
            value = repeat_kv(value, self.num_heads).transpose(1, 2)
