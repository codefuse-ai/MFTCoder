...
class BaichuanAttention(torch.nn.Module):
    ...
    def __init__(self, config: BaichuanConfig):
        super().__init__()
        self.config = config
        self.use_xformers = False # config.use_xformers
        self.use_triton = False # config.use_triton
        self.use_mqa = False # config.use_mqa
    ...
    def forward(
        ...
        if not self.use_mqa:
            ...
            # MQA
            query_states = q_proj(hidden_states).view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
            key_states = k_proj(hidden_states).view(bsz, q_len, 1, self.head_dim).transpose(1, 2)
            value_states = v_proj(hidden_states).view(bsz, q_len, 1, self.head_dim).transpose(1, 2)
        ...
        if self.use_xformers:
            ...
            print(f"shape of attention mask before: {attention_mask.shape}")
            # [query_states.shape[0], query_states.shape[2], query_states.shape[1], key_states.shape[1]]
            attention_mask = attention_mask.unsqueeze(0).repeat(bsz, 1, 1, 1).view(bsz, self.num_heads, q_len, kv_seq_len).contiguous()
            print(f"shape of attention mask after: {attention_mask.shape}")
            attn_output = xformers.ops.memory_efficient_attention(query_states, key_states, value_states, 
                                                                attn_bias=xformers.ops.fmha.attn_bias.LowerTriangularMaskWithTensorBias(attention_mask),
                                                                )
            ...
            """
            attention_mask = attention_mask.unsqueeze(0)[:, :, 0:1, :]
            query_states = query_states.transpose(1, 2)
            key_states = key_states.transpose(1, 2)
            value_states = value_states.transpose(1, 2)
            if self.use_mqa:
                key_states = key_states.repeat(1, 1, self.num_heads, 1)
                value_states = value_states.repeat(1, 1, self.nnum_heads, 1)
            attn_output = flash_attn_func(query_states, key_states, value_states, attention_mask, True)
            attn_weights = None
        else:
